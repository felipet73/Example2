...
import { Stair } from './components/Gradas'
...
function Item({ index, position, scale, c = new THREE.Color(), ...props }) {
  ...
  useFrame((state, delta) => {
    ...
    ref.current.material.scale[1] = ref.current.scale.y = damp(ref.current.scale.y, clicked === index ? 7 : 14 + y, 8, delta)
    ...
    ref.current.material.color.lerp(c.set(hovered || clicked === index ? 'white' : 'rgb(0,127,127)'), hovered ? 0.3 : 0.1)
  })
  return <Image ref={ref} {...props} position={position} scale={scale} onClick={click} onPointerOver={over} onPointerOut={out} />  
}

function Items({ w = 1.3, gap = 0.15 }) {
  ...
  return (
    <ScrollControls horizontal damping={10} pages={(width - xW + urls.length * xW) / width}>
      <Minimap />
      <Scroll>
        {urls.map((url, i) => <Item key={i} index={i} position={[i * xW+2, 0, 0]} scale={[w, 4, 1]} url={url} />) /* prettier-ignore */}
      </Scroll>
    </ScrollControls>
  )
}
...
export const Tiles = () => (
  <Canvas style={{zIndex:'999'}} gl={{ antialias: false }} dpr={[1, 1.5]} onPointerMissed={() => (state.clicked = null)}>
    <Items />
  </Canvas>
)
