...
import { addEstructura, cleanDataChartAPU, eliminaEstructura, eliminarXCodigo, guardarEstructura, limpiaAsociado, modificaEstructura, selectAPUS, selectAsociados } from "../actions/proyects.actions";
import TreeList, {
	Pager,
	Paging,
	Editing,
	...
	function itemClick(e) {
		if (!e.itemData.items) {
			...
			if (e.itemData.text === 'Agregar') {
				...
				if (proyects.Propiedades.length===1){

					//alert(proyects.DataAsociado.length)
					if (proyects.DataAsociado.length===0){

						Swal.fire({
							title: 'Error!',
							text: 'No ha seleccionado Asociados',
							icon: 'error',
							confirmButtonText: 'Ok'
						})
						return;
						...
						//dispatch(selectAsociados('', '', '', ''));
						dispatch(limpiaAsociado());
						setTimeout(() => {
							dispatch(selectAsociados(proyects.DatosPresupuesto[0].CodPresupuesto, proyects.Sub_sel, itemSel?.Item, ''));	
						}, 300);
		...
		return (
		  <>
			<div style={{ padding:'7px'}} >
			  <CheckBox
				defaultValue={datos.Mostrar==='true' ? true:false}
				...
				onValueChange={(e)=>{
				 ...
				 if (e)
				  ...
				  datos.Mostrar='false';
				  dispatch(modificaEstructura(datos));
			...
			<TreeList
				...
				onCellClick={(e) => {
					if (e.columnIndex === 2 && e.data) {
						...
						dispatch(modificaEstructura(e.data));
					}
					console.log('click en celda'); console.log(e)
